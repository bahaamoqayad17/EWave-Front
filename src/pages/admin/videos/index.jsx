...
const Page = () => {
  ...
  const { videos, loading, count } = useSelector((state) => state.videos);
  const getPagination = (page, limit) => {
    page++;
    dispatch(fetchVideos({ page, limit }));
  };
  ...
  const search = (e) => {
    ...
    if (e.key === "Enter") {
      if (value) {
        dispatch(fetchVideos({ title: value }));
      } else {
  ...
  return (
    <>
      <Head>
        <title>{`${process.env.APP_NAME} | Videos`}</title>
      </Head>
      <Box
        component="main"
        sx={{
